...
void *sha256_calc(CksumIOCtx *io_ctx) {
  ...
  /* Initialize the computation context.  */
  sha256_init_ctx(&ctx);
  ctx.notify_progress = io_ctx->notify_progress;
  ctx.io_ctx = io_ctx;

  /* Process whole buffer but last len % 64 bytes.  */
  sha256_process_bytes(io_ctx->msg_buf, io_ctx->msg_len, &ctx);

  /* Put result in desired memory area.  */
  return sha256_finish_ctx(&ctx, io_ctx->result_buf);
}
...
void *sha224_calc(CksumIOCtx *io_ctx) {
  ...
  /* Put result in desired memory area.  */
  return sha224_finish_ctx(&ctx, io_ctx->result_buf);
}
...
#define M(I)                                                                 \
  (tm = S1(x[(I - 2) & 0x0f]) + x[(I - 7) & 0x0f] + S0(x[(I - 15) & 0x0f]) + \
        x[I & 0x0f],                                                         \
   x[I & 0x0f] = tm)
